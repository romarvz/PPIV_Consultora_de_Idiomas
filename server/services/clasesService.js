...
/**
 * Obtener clase por ID con información completa
 */
...
exports.updateClase = async (claseId, updateData) => {
  ...
  if (updateData.fechaHora || updateData.duracionMinutos) {
    ...
    const disponible = await Clase.verificarDisponibilidadProfesor(
      ...
      fechaHora,
      duracion,
      claseId // Excluir esta clase de la verificación
    );
  ...
  // Actualizar eventos de calendario si cambió la fecha
...
/**
 * Obtener próximas clases de un usuario
 */
...
exports.completarClase = async (claseId) => {
  ...
  if (clase.estado === 'completada') {
    throw new Error('La clase ya está completada');
  }
  ...
  // Completar clase (esto actualiza automáticamente el progreso)
...
/**
 * Obtener estadísticas de un curso (clases)
 */
